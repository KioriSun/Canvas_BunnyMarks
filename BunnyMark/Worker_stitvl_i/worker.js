...
onmessage = (msg) => {
    ...
    switch (type) {
        // "input"
        case 0:
            item = new Float32Array(msg.data[1])
            // console.log("input ", item)
            genBunnies(item[0], item[1]);
            break;
        // "cSize"
        case 1:
            item = new Uint16Array(msg.data[1])
            // console.log("cSize ", item)
...
function bunUpdate() {
    ...
    const array = new Float32Array(coords)
    postMessage([bunnyCount, array.buffer], [array.buffer])
}
