...
function loadIMG() {
    ...
    // 1 = canvas size
    const cSize = new Float32Array([1, cWidth, cHeight, bWidth, bHeight])
    Thread.postMessage(cSize.buffer, [cSize.buffer])
    console.log("loaded")
    ...
    canvas.addEventListener("mousedown", (e) => {
        ...
        // 0 = input msgs
        const input = new Float32Array([0, offsetX, offsetY])
        Thread.postMessage(input.buffer, [input.buffer])
...
Thread.onmessage = (msg) => {
    ...
    coords = new Float32Array(msg.data)
    bunnyCount = coords[0]
    threadRender()
    // requestAnimationFrame(render)
}
